    ...
    <div class="card-form-and-image" :style="{flexDirection: direction}">
      ...
      <div class="credit-card-form">
        ...
        <div class="field-group">
          <div class="field">
            <label for="expirationMonth">{{ trans.expiration.label }} (mm/{{ isTwoDigitsYear ? 'yy' : 'yyyy' }})</label>
            <input type="text"
...
import SecurityCode from './components/singles/SingleSecurityCode'
import UnknownIcon from './components/singles/SingleUnknown'
import {
  isValid,
...
export default {
  ...
  methods: {
    ...
    defineMasks() {
      ...
      this.expirationMonthMask = new IMask(this.$refs.expiration, expirationMask(this.isTwoDigitsYear))
    ...
    setMasksListeners() {
      ...
      this.expirationMonthMask.on('accept', () => {
        this.form.expiration = this.expirationMonthMask.value
        const expirationArr = this.form.expiration.split(' / ')
        let expirationMonth = '';
        let expirationYear = ''
        if (expirationArr.length < 2) {
          this.creditExpirationError = ''
          return;
        }
        expirationMonth = expirationArr[0]
        expirationYear = expirationArr[1]
        ...
        if (!isExpirationDateValid(expirationMonth, '20' + expirationYear)) {
          this.creditExpirationError = 'check-error'
        } else {
          this.creditExpirationError = ''
      ...
      this.securityCodeMask.on('accept', () => {
        ...
        if (!isSecurityCodeValid(this.cardNumberMask.value, this.securityCodeMask.value) && this.form.security.length >= 3) {
          this.creditCvcError = 'check-error'
        } else {
          this.creditCvcError = ''
        }
      })
      ...
      this.cardNumberMask.on('accept', () => {
        ...
        if (!isValid(cardNumber) && cardNumber.length === this.cardNumberMaxLength) {
          this.creditNumberError = 'check-error'
        } else {
  ...
  computed: {
    ...
    fields() {
      return [this.form.cardNumber, this.form.expiration, this.form.security].join('')
    }
  },
  watch: {
    fields() {
      let isValid = true
      if (this.creditNumberError === ''
          && this.creditExpirationError === ''
          && this.creditCvcError === '') {

        if (this.form.cardNumber.length < this.cardNumberMaxLength) {
          isValid = false
        }
        ...
        if (this.form.security.length < 3) {
          isValid = false
        }
        if (isValid) {
